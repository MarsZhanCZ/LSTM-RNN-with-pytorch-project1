...
def create_dataset(data, days_for_train=5) -> (np.array, np.array): #->用于指示函数返回的类型
    ...
    for i in range(10, len(data)-1):#0,1,2...len-1，用于生成label
        if data[i+1] > data[i]:
...
# 将数据改变形状，RNN 读入的数据维度是 (seq_len, batch_size, feature_size),batch_first=True变成(batch_size, seq_len, feature_size)
#train_x = train_x.reshape(-1, 1, DAYS_FOR_TRAIN)
train_x = train_x.reshape(-1, DAYS_FOR_TRAIN, 1)
train_y = train_y.reshape(-1, 1, 2)#2维度标签
...
class LSTM_Regression(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size=2, num_layers=2, num_class=2):#output size类数
        super().__init__()
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first= True)#(DaysForTrain?2, 16, 2)
        self.fc = nn.Linear(hidden_size, output_size)
        self.classifier = nn.Linear(hidden_size, num_class)

    def forward(self, _x):
        #print('train_x的维度是：', _x.shape)
        x, _ = self.lstm(_x)  # _x is input, size (seq_len, batch, input_size),batch_first=True:(batch_seq_len,input_size)
        s, b, h = x.shape  # x is output, size (seq_len, batch, hidden_size),batch_first=True:(batch_seq_len,hidden_size)
...
model = LSTM_Regression(1, 16, output_size=2, num_layers=2, num_class=2)#如果是要多维输出则output_size=n。第一的参数是feature_size
